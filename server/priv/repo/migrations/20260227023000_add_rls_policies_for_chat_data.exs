defmodule Vibe.Repo.Migrations.AddRlsPoliciesForChatData do
  use Ecto.Migration
  ...
  def up do
    execute("""
    CREATE OR REPLACE FUNCTION vibe_current_user_id() RETURNS uuid
    ...
    BEGIN
      ...
      IF uid IS NULL THEN
        -- Compatibility mode: current stack does not always set app.current_user_id.
        RETURN true;
      END IF;
    ...
    BEGIN
      uid := vibe_current_user_id();
      IF uid IS NULL THEN
        -- Compatibility mode: keep existing behavior when user context is not set.
        RETURN true;
      END IF;
    ...
    execute("ALTER TABLE group_agents ENABLE ROW LEVEL SECURITY")
    execute("ALTER TABLE group_agent_memory ENABLE ROW LEVEL SECURITY")
  ...
  def down do
    ...
    execute("ALTER TABLE group_agents DISABLE ROW LEVEL SECURITY")
    execute("ALTER TABLE group_agent_memory DISABLE ROW LEVEL SECURITY")

    execute("DROP FUNCTION IF EXISTS vibe_is_chat_admin(text)")
    execute("DROP FUNCTION IF EXISTS vibe_is_chat_participant(text)")
    execute("DROP FUNCTION IF EXISTS vibe_current_user_id()")
  end
end
