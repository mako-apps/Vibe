...
const toLabel = (v: PrivacyValue) => (v === 'everybody' ? 'Everybody' : v === 'contacts' ? 'My Contacts' : 'Nobody');
...
export default function PrivacyChoiceScreen() {
    ...
    const title = (params.title || 'Privacy') as string;
    const key = (params.key || '') as string;

    const currentValue = useMemo(() => {
        const u: any = user || {};
        return (u[key] as PrivacyValue) || 'everybody';
    }, [user, key]);

    const setValue = async (v: PrivacyValue) => {
        if (!key) return;
        await updateProfileInfo({ [key]: v } as any);
        router.back();
    };

    const options: PrivacyValue[] = ['everybody', 'contacts', 'nobody'];
                ...
                <View style={{ paddingTop: insets.top + 80, paddingHorizontal: 16 }}>
                    <View style={[styles.group, { backgroundColor: effectiveTheme === 'light' ? '#fff' : colors.card }]}>
                        {options.map((v, idx) => {
                            ...
                            return (
                                <View key={v}>
                                    <TouchableOpacity onPress={() => setValue(v)} activeOpacity={0.8} style={styles.row}>
                                        <Text style={[styles.rowLabel, { color: colors.text }]}>{toLabel(v)}</Text>
