...
import com.mohammadshayani.vibe.home.ChatHomeUndoBannerView
...
class ChatNativeHomeListView(
  ...
  private val swipeRefreshLayout = HorizontalAwareSwipeRefreshLayout(context)
  private val recyclerView = RecyclerView(context)
  private val undoBannerView = ChatHomeUndoBannerView(context)
  ...
  init {
    ...
    rootContainer.addView(
      ...
      FrameLayout.LayoutParams(
        FrameLayout.LayoutParams.MATCH_PARENT,
        dp(ChatHomeUndoBannerView.PREFERRED_HEIGHT_DP),
      ).apply {
  ...
  fun setRows(rawRows: List<Map<String, Any?>>) {
    rows = parseChatHomeRows(rawRows, context)
  ...
  private fun resolvedPresenceRow(row: ChatHomeListRow): ChatHomeListRow {
   ...
   */
  private fun resolvePreviewFromEngine(row: ChatHomeListRow): String? {
  ...
  private fun showNativePreview(row: ChatHomeListRow) {
  ...
  private fun leadingSwipeSpecs(row: ChatHomeListRow): List<ChatNativeHomeSwipeSpec> {
    val hasUnread = row.unreadCount > 0 || row.markedUnread
    return listOf(
  ...
  private fun trailingSwipeSpecs(row: ChatHomeListRow): List<ChatNativeHomeSwipeSpec> {
    return listOf(
      ChatNativeHomeSwipeSpec(
        eventType = "swipeMute",
  ...
  private fun clampSwipeDx(dX: Float, itemView: View, row: ChatHomeListRow): Float {
    return if (dX > 0f) {
  ...
  private fun maybeEmitSwipeHaptic(position: Int, itemView: View, dX: Float, row: ChatHomeListRow) {
  ...
  private fun drawSwipeActions(canvas: Canvas, itemView: View, dX: Float, row: ChatHomeListRow) {
    if (dX > 0f) {
  ...
  private fun drawLeadingSwipeActions(canvas: Canvas, itemView: View, dX: Float, row: ChatHomeListRow) {
  ...
  private fun drawTrailingSwipeActions(canvas: Canvas, itemView: View, dX: Float, row: ChatHomeListRow) {
  ...
  private fun leadingOpenWidthPx(row: ChatHomeListRow): Float =
    leadingSwipeSpecs(row).size * leadingActionWidthPx()

  private fun trailingOpenWidthPx(row: ChatHomeListRow): Float =
  ...
  private class HomeListAdapter(
    private val onPress: (ChatHomeListRow) -> Unit,
    private val onLongPress: (ChatHomeListRow, View) -> Boolean,
  ) : RecyclerView.Adapter<HomeListAdapter.ViewHolder>() {
    private val rows = ArrayList<ChatHomeListRow>()
    private var isDark: Boolean = false
    private var previewAppearance: Map<String, Any?> = emptyMap()

    fun submitRows(nextRows: List<ChatHomeListRow>) {
      rows.clear()
    ...
    fun getRow(index: Int): ChatHomeListRow? = rows.getOrNull(index)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
      return ViewHolder(ChatHomeCardView(parent.context))
    }
    ...
    private fun resolveAvatarGradientColors(row: ChatHomeListRow): IntArray? {
    ...
    class ViewHolder(val card: ChatHomeCardView) : RecyclerView.ViewHolder(card)
  }
}

private class HorizontalAwareSwipeRefreshLayout(context: Context) : SwipeRefreshLayout(context) {
  ...
  override fun onInterceptTouchEvent(ev: MotionEvent): Boolean {
    when (ev.actionMasked) {
      ...
      MotionEvent.ACTION_MOVE -> {
        val dx = ev.x - downX
        val dy = ev.y - downY
        if (!horizontalGesture && abs(dx) > touchSlop && abs(dx) > abs(dy) * 1.12f) {
